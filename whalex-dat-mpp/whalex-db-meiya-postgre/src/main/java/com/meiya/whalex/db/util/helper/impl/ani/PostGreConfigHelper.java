...
@DbHelper(dbType = DbResourceEnum.postgre, version = DbVersionEnum.POSTGRE_9_3, cloudVendors = CloudVendorsEnum.OPEN)
@Slf4j
public class PostGreConfigHelper extends BasePostGreConfigHelper {
    @Override
    public QueryRunner initDbConnect(BasePostGreDatabaseInfo databaseConf, BasePostGreTableInfo tableConf) {
        ...
        if(StringUtils.isNotBlank(databaseConf.getDatabase())) {
            ...
            if(StringUtils.isNotBlank(databaseConf.getSchema())) {
                sb.append("?currentSchema=").append("\"").append(databaseConf.getSchema()).append("\"");
            }
        }
