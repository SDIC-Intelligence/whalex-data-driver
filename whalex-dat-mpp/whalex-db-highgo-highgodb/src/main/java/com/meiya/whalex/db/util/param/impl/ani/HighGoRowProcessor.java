 ...
 */
@Slf4j
public class HighGoRowProcessor extends BasicRowProcessor {
    ...
    public Map<String, Object> toMap(ResultSet rs) throws SQLException {
        ...
        for(int i = 1; i <= cols; ++i) {
            ...
            Object value = null;
            try {
                if (StringUtils.equalsIgnoreCase(columnTypeName, BasePostGreFieldTypeEnum.MONEY.getDbFieldType())) {
                    value = new BigDecimal(StringUtils.replaceEach(rs.getString(i), new String[]{"$", ","}, new String[]{"", ""}));
                } else {
                    value = rs.getObject(i);
                    // 处理pg库不存在的字段类型, 比如nvarchar2(libra存在)
                    // 只取里面的值
                    if (value instanceof PGpoint) {
                        value = ((PGpoint) value).x + "," + ((PGpoint) value).y;
                    } else if (value instanceof PGobject) {
                        if (log.isDebugEnabled()) {
                            log.debug("object is pgObject [type: {}], only read value", ((PGobject) value).getType());
                        }
