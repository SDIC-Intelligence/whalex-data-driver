...
public class BaseHBaseConfigHelper<S extends HBaseTemplate
    ...
    public D initDbModuleConfig(DatabaseConf conf) {
        ...
        // 存储证书目录
        String parentDir = dbJsonMap.get("parentDir");
        ...
        hBaseDatabaseInfo.setParentDir(parentDir);
    ...
    public T initTableConfig(TableConf conf) {
        ...
        if (StringUtils.isBlank(tableJson) || StringUtils.isBlank(tableName)) {
...
//        conf.setInt("hbase.client.keyvalue.maxsize", 50 * 1024 * 1024);
        // 设置连接池大小
