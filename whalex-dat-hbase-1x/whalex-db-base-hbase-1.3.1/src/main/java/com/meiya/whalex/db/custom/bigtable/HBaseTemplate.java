...
//	}
     ...
     * @throws IOException
     */
    public void createTable(String tableName, List<Map<String, String>> tableDesMapList) throws IOException {
        tableName = getNameSpaceTableName(tableName);
        Admin admin = hTablePool.getAdmin();
     ...
     * @throws BusinessException 异常
     */
    public KeyValue getKV(String tableName, byte[] row, final byte[] family,
                          final byte[] qualifier) throws BusinessException {
        final Get get = new Get(row);
        get.addColumn(family, qualifier);

        return execute(tableName, new TableTemplateCallback<KeyValue>() {
            @Override
            public KeyValue doWithTable(Table htable) throws BusinessException,
                    IOException {
                Result result = htable.get(get);
                if (result.isEmpty()) {
                    return null;
    ...
    /**
     * 执行一个针对 row + columnFamily 的 Get 操作。
     *
     * @param tableName    表名
     * @param row          列
     ...
     * @throws BusinessException 异常
     */
    public List<KeyValue> get(String tableName, byte[] row, byte[] columnFamily) {
        return this.get(tableName, row, columnFamily, new ResultCallback<List<KeyValue>>() {
            @Override
            public List<KeyValue> doInRow(Result result) throws BusinessException, IOException {
     ...
     * @throws BusinessException 异常
     */
    public List<KeyValue> get(String tableName, byte[] row)
            throws BusinessException {
        return this.get(tableName, row, new ResultCallback<List<KeyValue>>() {
            @Override
            public List<KeyValue> doInRow(Result result)
    ...
    public List<Map<String, Object>> getSchemaInfo(String tableName) throws IOException {
        ...
        try {
            ...
            for (HColumnDescriptor hColumnDescriptor : columnFamilies) {
                ...
                map.put("columnFamily", name);
                map.put("ttl", hColumnDescriptor.getTimeToLive());
                map.put("versions", hColumnDescriptor.getMaxVersions());
                map.put("min_versions", hColumnDescriptor.getMinVersions());
                map.put("in_memory", hColumnDescriptor.isInMemory());
                map.put("blocksize", hColumnDescriptor.getBlocksize());
                map.put("blockcache", hColumnDescriptor.isBlockCacheEnabled());
                map.put("evict_blocks_on_close", hColumnDescriptor.isEvictBlocksOnClose());
                KeepDeletedCells keepDeletedCells = hColumnDescriptor.getKeepDeletedCells();
                if(keepDeletedCells != null) {
                    map.put("keep_deleted_cells", keepDeletedCells.name());
                }
                map.put("cache_data_on_write", hColumnDescriptor.isCacheDataOnWrite());
                ...
                map.put("replication_scope", hColumnDescriptor.getScope());
                ...
                map.put("cache_index_on_write", hColumnDescriptor.isCacheIndexesOnWrite());
                map.put("cache_blooms_on_write", hColumnDescriptor.isCacheBloomsOnWrite());
                map.put("prefetch_blocks_on_open", hColumnDescriptor.isPrefetchBlocksOnOpen());
