...
public class BaseHBaseServiceImpl<S extends HBaseTemplate, Q extends HBaseHandler, D extends HBaseDatabaseInfo, T extends HBaseTableInfo, C extends HBaseCursorCache>
    ...
    private static final String COPROCESSOR_CLASS = "org.apache.hadoop.hbase.coprocessor.AggregateImplementation";
    ...
    protected QueryMethodResult countMethod(S connect, Q queryEntity, D databaseConf, T tableConf) throws Exception {
        throw new BusinessException(ExceptionCode.METHOD_NOT_IMPLEMENTED_EXCEPTION, "hBaseServiceImpl.countMethod");
        /*HBaseHandler.HBaseQuery hBaseQuery = queryEntity.getQuery();

        // 获取分表表名
        List<String> periodTableNames = DatatypePeriodUtils.getPeriodTableNames(tableConf.getTableName(), tableConf.getPeriodType(), hBaseQuery.getStartTime(), hBaseQuery.getStopTime(), DbResourceEnum.hbase, tableConf.getPrePeriodValue());
        queryEntity.setQueryStr(transitionQueryStr("SCAN", periodTableNames, hBaseQuery));
        long total = 0;
        ...
        for (int i = 0; i < periodTableNames.size(); i++) {
            ...
            HTableDescriptor descriptor = admin.getTableDescriptor(tableNameForHBase);
            ...
            if (!descriptor.hasCoprocessor(coprocessorClass)) {
                ...
                try {
                    ...
                    admin.modifyTable(tableNameForHBase, descriptor);
                } finally {
            ...
            try {
                stopWatch.start();
                Scan scan = new Scan();
                AggregationClient aggregationClient = new AggregationClient(connect.getHTablePool().getConfiguration());
    ...
    protected QueryMethodResult createTableMethod(S connect, Q queryEntity, D databaseConf, T tableConf) throws Exception {
        ...
        for (int i = 0; i < tableNameList.size(); i++) {
            ...
            try {
                ...
                connect.createTable(tableName, schemaMap);
            } catch (Exception e) {
...
//                .append(splitalgoBuilder.toString())
                ...
                if (!family.equalsIgnoreCase(CommonConstant.ROW_KEY) && value instanceof Map) {
                    ...
                    for (Map.Entry<String, Object> qualifierEntry : qualifierMap.entrySet()) {
                        ...
                        if (qualifierEntryValue instanceof byte[]) {
                            qualifierEntryValue = cn.hutool.core.codec.Base64.encode((byte[]) qualifierEntryValue);
                        }
                        if(builder.length() > 0) {
                            builder.append("\n");
                        }
    ...
    protected QueryMethodResult updateMethod(S connect, Q queryEntity, D databaseConf, T tableConf) throws Exception {
        throw new BusinessException(ExceptionCode.METHOD_NOT_IMPLEMENTED_EXCEPTION, "hBaseServiceImpl.updateMethod");
    }

    @Override
    protected QueryMethodResult delMethod(S connect, Q queryEntity, D databaseConf, T tableConf) throws Exception {
        throw new BusinessException(ExceptionCode.METHOD_NOT_IMPLEMENTED_EXCEPTION, "hBaseServiceImpl.delMethod");
    }
...
//            schema = BaseHBaseConfigHelper.DEFAULT_BC_FAMILY_SCHEME;
//        }
//        List<Map<String, Object>> schemaMap = JsonUtil.jsonStrToObject(schema, List.class);
//        Map<String, Object> regionsMap = null;
//        for (Map<String, Object> tableDesMap : schemaMap) {
//            if (tableDesMap.get("SPLITALGO") != null) {
//                regionsMap = tableDesMap;
//                break;
//            }
...
//        }
//        // 设置分区个数
//        regionsMap.put("NUMREGIONS", String.valueOf(tableConf.getRegionCount()));
        ////describe 'tableName'
        ...
        List<Map<String, Object>> schemaInfo = connect.getSchemaInfo(tableConf.getTableName());
    ...
    protected QueryCursorMethodResult queryCursorMethod(S connect, Q queryEntity, D databaseConf, T tableConf, C cursorCache, Consumer<Map<String, Object>> consumer, Boolean rollAllData) throws Exception {
        ...
        if (cursorCache == null) {
            ...
            // 匹配到缓存
            // 从缓存中获取批量数
            query.setBatchSize(cursorCache.getBatchSize());
            remainSize = cursorCache.getBatchSize();
    ...
    protected QueryMethodResult alterTableMethod(S connect, Q queryEntity, D databaseConf, T tableConf) throws Exception {
        throw new BusinessException(ExceptionCode.METHOD_NOT_IMPLEMENTED_EXCEPTION, "HBaseServiceImpl.alterTableMethod");
    }
    ...
    protected QueryMethodResult queryTableInformationMethod(S connect, D databaseConf, T tableConf) throws Exception {
        ...
        // 获取表信息
        String nameSpaceTableName = connect.getNameSpaceTableName(tableConf.getTableName());
        HTableDescriptor tableDescriptor = admin.getTableDescriptor(TableName.valueOf(nameSpaceTableName));
        int regionReplication = tableDescriptor.getRegionReplication();
        // 获取表region信息
        List<HRegionInfo> tableRegions = admin.getTableRegions(TableName.valueOf(tableConf.getTableName()));
        Map<String, HBaseTableInformation.HBaseRegionInfo> regionInfoMap = new HashMap<>(tableRegions.size());
        for (int i = 0; i < tableRegions.size(); i++) {
            HRegionInfo hRegionInfo = tableRegions.get(i);
            String encodedName = hRegionInfo.getEncodedName();
            String endKey = org.apache.hadoop.hbase.util.Base64.encodeBytes(hRegionInfo.getEndKey());
            String startKey = org.apache.hadoop.hbase.util.Base64.encodeBytes(hRegionInfo.getStartKey());
            long regionId = hRegionInfo.getRegionId();
