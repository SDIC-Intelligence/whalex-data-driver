...
import com.meiya.whalex.db.util.helper.impl.document.PseudoClusterMongoClient;
...
public class BaseMongoServiceImpl extends BaseMongoRawStatementModuleImpl {
    ...
    protected QueryMethodResult queryMethod(MongoClient mongoClient, MongoHandle mongoHandle, MongoDatabaseInfo databaseConf, MongoTableInfo tableConf) throws Exception {
        ...
        if (mongoClient instanceof PseudoClusterMongoClient) {
            long total = 0;
            int limit = mongoHandle.getMongoQuery().getLimit();
            boolean count = mongoHandle.getMongoQuery().getCount();
            PseudoClusterMongoClient pseudoClusterMongoClient = (PseudoClusterMongoClient) mongoClient;
            List<MongoClient> mongoClientList = pseudoClusterMongoClient.getMongoClientList();
            for (int i = 0; i < mongoClientList.size(); i++) {
                MongoClient standOneClient = mongoClientList.get(i);
                QueryCursorMethodResult queryCursorMethodResult = queryCursorMethod(standOneClient, mongoHandle, databaseConf, tableConf, null, (rowMap) -> {
                    result.add(rowMap);
                }, Boolean.TRUE);
                if (count) {
                    total = +queryCursorMethodResult.getTotal();
                    if (result.size() >= limit) {
                        List<MongoClient> mongoClients = mongoClientList.subList(i, mongoClientList.size());
                        for (int t = 0; t < mongoClients.size(); t++) {
                            MongoClient _mongoClient = mongoClients.get(t);
                            QueryMethodResult queryMethodResult = countMethod(_mongoClient, mongoHandle, databaseConf, tableConf);
                            total = +queryMethodResult.getTotal();
                        }
                        break;
                    }
                } else {
            ...
            cursorMethodResult = new QueryCursorMethodResult<>();
            cursorMethodResult.setTotal(total);
        } else {
    ...
    private QueryMethodResult queryAggregate(MongoClient mongoClient, MongoHandle mongoHandle, MongoDatabaseInfo databaseConf, MongoTableInfo tableConf) {
        ...
        String queryStr = transitionAggQueryStr(tableConf.getTableName(), mongoQuery);
        mongoHandle.setQueryStr(queryStr);
    ...
    protected QueryMethodResult countMethod(MongoClient connect, MongoHandle mongoHandle, MongoDatabaseInfo databaseConf, MongoTableInfo tableConf) throws Exception {
        ...
        MongoHandle.MongoQuery mongoQuery = mongoHandle.getMongoQuery();
        // 记录查询语句
        String queryStr = transitionQueryStr(tableConf.getTableName(), mongoHandle.getMongoQuery());
        mongoHandle.setQueryStr(queryStr);
        DatabaseExecuteStatementLog.set(queryStr);
        ...
        if (connect instanceof PseudoClusterMongoClient) {
            ...
            for (int i = 0; i < mongoClientList.size(); i++) {
                ...
                MongoDatabase database = mongoClient.getDatabase(databaseConf.getDatabaseName());
                MongoCollection<Document> collection = database.getCollection(tableConf.getTableName());
                long count = collection.count(mongoQuery.getQuery());
                _total = +count;
            }
            queryMethodResult.setTotal(_total);
        } else {
    ...
    protected QueryMethodResult testConnectMethod(MongoClient connect, MongoDatabaseInfo databaseConf) throws Exception {
        DatabaseExecuteStatementLog.set("show collections");
        if (connect instanceof PseudoClusterMongoClient) {
            PseudoClusterMongoClient pseudoClusterMongoClient = (PseudoClusterMongoClient) connect;
            List<MongoClient> mongoClientList = pseudoClusterMongoClient.getMongoClientList();
            for (int i = 0; i < mongoClientList.size(); i++) {
                MongoClient mongoClient = mongoClientList.get(i);
                testConnectMethod(mongoClient, databaseConf);
            }
        } else {
            List<String> databaseNames = connect.getDatabaseNames();
        }
    ...
    protected QueryMethodResult showTablesMethod(MongoClient connect, MongoHandle mongoHandle, MongoDatabaseInfo databaseConf) throws Exception {
        ...
        if (connect instanceof PseudoClusterMongoClient) {
            PseudoClusterMongoClient pseudoClusterMongoClient = (PseudoClusterMongoClient) connect;
            connect = pseudoClusterMongoClient.getMongoClientList().get(0);
        }
        try {
            MongoDatabase database = connect.getDatabase(databaseConf.getDatabaseName());
            DatabaseExecuteStatementLog.set("show collections");
    ...
    protected QueryMethodResult getIndexesMethod(MongoClient connect, MongoDatabaseInfo databaseConf, MongoTableInfo tableConf) throws Exception {
        ...
        try {
            MongoDatabase database = connect.getDatabase(databaseConf.getDatabaseName());
            MongoCollection<Document> collection = database.getCollection(tableConf.getTableName());
            DatabaseExecuteStatementLog.set("db." + tableConf.getTableName() + ".listIndexes()");
    ...
    protected QueryMethodResult createTableMethod(MongoClient connect, MongoHandle mongoHandle, MongoDatabaseInfo databaseConf, MongoTableInfo tableConf) throws Exception {
        // 判断是否是集群
        if (ClusterType.SHARDED.equals(databaseConf.getClusterType()) && !databaseConf.isPseudoCluster()) {
            // 在目标数据库创建索引
            MongoDatabase database = connect.getDatabase(databaseConf.getDatabaseName());
            MongoCollection<Document> collection = database.getCollection(tableConf.getTableName());
    ...
    protected QueryMethodResult dropTableMethod(MongoClient connect, MongoHandle mongoHandle, MongoDatabaseInfo databaseConf, MongoTableInfo tableConf) throws Exception {
        MongoDatabase database = connect.getDatabase(databaseConf.getDatabaseName());
        DatabaseExecuteStatementLog.set("db." + tableConf.getTableName() + ".drop()");
    ...
    protected QueryMethodResult deleteIndexMethod(MongoClient connect, MongoHandle mongoHandle, MongoDatabaseInfo databaseConf, MongoTableInfo tableConf) throws Exception {
        ...
        if (StringUtils.isNotBlank(indexName)) {
            DatabaseExecuteStatementLog.set("db." + tableConf.getTableName() + ".dropIndex(" + indexName + ")");
            database.getCollection(tableConf.getTableName()).dropIndex(indexName);
        } else {
            DatabaseExecuteStatementLog.set("db." + tableConf.getTableName() + ".dropIndex(" + JsonUtil.objectToStr(mongoHandle.getMongoIndex().getIndex()) + ")");
    ...
    protected QueryMethodResult createIndexMethod(MongoClient connect, MongoHandle mongoHandle, MongoDatabaseInfo databaseConf, MongoTableInfo tableConf) throws Exception {
        MongoDatabase database = connect.getDatabase(databaseConf.getDatabaseName());
        MongoCollection<Document> collection = database.getCollection(tableConf.getTableName());
        DatabaseExecuteStatementLog.set("db." + tableConf.getTableName() + ".createIndex(" + JsonUtil.objectToStr(mongoHandle.getMongoIndex().getIndex()) + ", " + JsonUtil.objectToStr(mongoHandle.getMongoIndex().getOptions()) + ")");
    ...
    protected QueryMethodResult insertMethod(MongoClient connect, MongoHandle mongoHandle, MongoDatabaseInfo databaseConf, MongoTableInfo tableConf) throws Exception {
        ...
        DatabaseExecuteStatementLog.set("db." + tableConf.getTableName() + ".insertMany(" + JsonUtil.objectToStr(mongoInsert.getInsertList()) + ")");
    ...
    protected QueryMethodResult updateMethod(MongoClient connect, MongoHandle mongoHandle, MongoDatabaseInfo databaseConf, MongoTableInfo tableConf) throws Exception {
        ...
        if (mongoUpdate.getMulti()) {
            DatabaseExecuteStatementLog.set("db." + tableConf.getTableName() + ".updateMany(" + JsonUtil.objectToStr(mongoUpdate.getQuery()) + ", " + JsonUtil.objectToStr(mongoUpdate.getUpdate()) + ", " + JsonUtil.objectToStr(mongoUpdate.getOptions()) + ")");
            updateResult = collection.updateMany(mongoUpdate.getQuery(), mongoUpdate.getUpdate(), mongoUpdate.getOptions());
        } else {
            DatabaseExecuteStatementLog.set("db." + tableConf.getTableName() + ".updateOne(" + JsonUtil.objectToStr(mongoUpdate.getQuery()) + ", " + JsonUtil.objectToStr(mongoUpdate.getUpdate()) + ", " + JsonUtil.objectToStr(mongoUpdate.getOptions()) + ")");
    ...
    protected QueryMethodResult delMethod(MongoClient connect, MongoHandle mongoHandle, MongoDatabaseInfo databaseConf, MongoTableInfo tableConf) throws Exception {
        ...
        collection = collection.withWriteConcern(writeConcern);
        DatabaseExecuteStatementLog.set("db." + tableConf.getTableName() + ".deleteMany( " + JsonUtil.objectToStr(mongoDel.getQuery()) + " )");
    ...
    protected QueryMethodResult querySchemaMethod(MongoClient connect, MongoDatabaseInfo databaseConf, MongoTableInfo tableConf) throws Exception {
        ...
        MongoDatabase database = connect.getDatabase(databaseConf.getDatabaseName());
        MongoCollection<Document> collection = database.getCollection(tableConf.getTableName());
        DatabaseExecuteStatementLog.set("db." + tableConf.getTableName() + ".find().limit(1)");
    ...
    protected QueryCursorMethodResult queryCursorMethod(MongoClient mongoClient, MongoHandle mongoHandle, MongoDatabaseInfo databaseConf, MongoTableInfo tableConf, MongoCursorCache cursorCache, Consumer<Map<String, Object>> consumer, Boolean rollAllData) throws Exception {
        ...
        if (cursorCache == null) {
            ...
            // 记录查询语句
            String queryStr = transitionQueryStr(tableConf.getTableName(), queryEntity);
            mongoHandle.setQueryStr(queryStr);
            DatabaseExecuteStatementLog.set(queryStr);
            // 通过游标查询
            MongoCursor<Document> cursor = null;
            ...
            if (isCount) {
                ...
                total = collection.count(null == queryCondition ? new BasicDBObject() : queryCondition, countOptions);
            }
            ...
            // 查询提示
            if (queryEntity.getHint() != null) {
                documents.modifiers(queryEntity.getHint());
            }
    ...
    protected QueryMethodResult saveOrUpdateBatchMethod(MongoClient connect, MongoHandle queryEntity, MongoDatabaseInfo databaseConf, MongoTableInfo tableConf) throws Exception {
        ...
        collection = collection.withWriteConcern(writeConcern);
        DatabaseExecuteStatementLog.set("db." + tableConf.getTableName() + ".bulkWrite(" + JsonUtil.objectToStr(updateOneModelList) + ")");
    ...
    protected QueryMethodResult alterTableMethod(MongoClient connect, MongoHandle queryEntity, MongoDatabaseInfo databaseConf, MongoTableInfo tableConf) throws Exception {
        MongoHandle.MongoAlterTable alterTable = queryEntity.getMongoAlterTable();
        ...
        if (CollectionUtil.isNotEmpty(addFields)) {
            DatabaseExecuteStatementLog.set("db." + tableConf.getTableName() + ".updateMany({}, " + alterTable.getAddFieldsJson() + ")");
            collection.updateMany(new Document(), Updates.combine(addFields));
        }

        if (CollectionUtil.isNotEmpty(updateFields)) {
            DatabaseExecuteStatementLog.set("db." + tableConf.getTableName() + ".updateMany({}, " + alterTable.getUpdateFieldsJson() + ")");
            collection.updateMany(new Document(), Updates.combine(updateFields));
        }

        if (CollectionUtil.isNotEmpty(delFields)) {
            DatabaseExecuteStatementLog.set("db." + tableConf.getTableName() + ".updateMany({}, " + alterTable.getDelFieldsJson() + ")");
            collection.updateMany(new Document(), Updates.combine(delFields));
        }

        if (renameCommand != null) {
            DatabaseExecuteStatementLog.set("db.runCommand(" + JsonUtil.objectToStr(renameCommand) + ")");
            MongoDatabase admin = connect.getDatabase("admin");
    ...
    protected QueryMethodResult queryListDatabaseMethod(MongoClient connect, MongoHandle queryEntity, MongoDatabaseInfo databaseConf, MongoTableInfo tableConf) throws Exception {
        ...
        //获取数据库列表
        DatabaseExecuteStatementLog.set("show dbs");
        List<String> databaseNames = connect.getDatabaseNames();

        //组装数据库列表
        List<Map<String, Object>> rows = new ArrayList<>(databaseNames.size());
    ...
    protected QueryMethodResult createDatabaseMethod(MongoClient connect, MongoHandle queryEntity, MongoDatabaseInfo databaseConf, MongoTableInfo tableConf) throws Exception {
        ...
        DatabaseExecuteStatementLog.set("use " + dbName);
    ...
    protected QueryMethodResult dropDatabaseMethod(MongoClient connect, MongoHandle queryEntity, MongoDatabaseInfo databaseConf, MongoTableInfo tableConf) throws Exception {
        ...
        DatabaseExecuteStatementLog.set("db.dropDatabase()");
        MongoDatabase database = connect.getDatabase(dbName);
        database.drop();
        return new QueryMethodResult(1, null);
    }
