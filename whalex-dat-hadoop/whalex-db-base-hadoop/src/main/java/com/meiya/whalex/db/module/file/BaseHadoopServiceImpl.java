...
public class BaseHadoopServiceImpl<S extends HDFSClient,
        Q extends HadoopHandler,
                ...
                Map<String, ParquetWriter<Group>> parquetWriterMap = writerMap.get(writerKey, new Function<String, Map<String, ParquetWriter<Group>>>() {
                    ...
                    public Map<String, ParquetWriter<Group>> apply(String s) {
                        try {
                            ...
                            file = file + "/" + UUID.randomUUID().toString().replaceAll("-", "") + "." + HadoopTableInfo.PARQUET_TYPE;
                ...
                Map<String, OrcWriter> orcWriterMap = writerMap.get(writerKey, new Function<String, Map<String, OrcWriter>>() {
                    ...
                    public Map<String, OrcWriter> apply(String s) {
                        try {
                            ...
                            file = file + "/" + UUID.randomUUID().toString().replaceAll("-", "") + "." + HadoopTableInfo.ORC_TYPE;
    ...
    protected QueryMethodResult queryMethod(S connect, Q queryEntity, D databaseConf, T tableConf) throws Exception {
        ...
        if (StringUtils.isBlank(fileType)) {
            ...
            if (!StrUtil.equalsAnyIgnoreCase(fileType, HadoopTableInfo.ORC_TYPE, HadoopTableInfo.PARQUET_TYPE)) {
        ...
        if (StrUtil.equalsIgnoreCase(fileType, HadoopTableInfo.ORC_TYPE)) {
            SchemaInfo<StandardStructObjectInspector> orcSchema = connect.getOrcSchema(databaseConf, tableConf);
            String partitionFiled = orcSchema.getPartitionFiled();
            String partitionPath = orcSchema.getPath();
            List<FileStatus> dataDic = ls.parallelStream().filter(dic -> !StrUtil.equalsAny(dic.getPath().getName(), ".meta", ".tmp"))
                    .collect(Collectors.toList());
            if (StringUtils.isNotBlank(partitionFiled)) {
                // 说明存在分区
                List<FileStatus> collect = dataDic.stream().filter(dic -> StringUtils.startsWithIgnoreCase("/" + dic.getPath().getName(), partitionPath)).collect(Collectors.toList());
                for (FileStatus uri : collect) {
                    List<FileStatus> parquetFiles = connect.ll(uri.getPath().toString());
                    if (CollectionUtil.isNotEmpty(parquetFiles)) {
                        for (FileStatus parquetFile : parquetFiles) {
                            String name = parquetFile.getPath().getName();
                            if (!StringUtils.endsWithIgnoreCase(name, HadoopTableInfo.ORC_TYPE)) {
                                continue;
                            }
                            long len = parquetFile.getLen();
                            if (len > 0) {
                                Path parquetFilePath = parquetFile.getPath();
                                List<Map<String, Object>> list = readOrc(parquetFilePath, connect, queryEntity.getQuery().getLimit(), orcSchema.getSchema());
                                return new QueryMethodResult(list.size(), list);
                            }
                        }
                    }
                }
            } else {
                for (FileStatus parquetFile : dataDic) {
                    String name = parquetFile.getPath().getName();
                    if (!StringUtils.endsWithIgnoreCase(name, HadoopTableInfo.ORC_TYPE)) {
                        continue;
                    }
            ...
            SchemaInfo<MessageType> parquetSchema = connect.getParquetSchema(databaseConf, tableConf);
            String partitionFiled = parquetSchema.getPartitionFiled();
            String partitionPath = parquetSchema.getPath();
            List<FileStatus> dataDic = ls.parallelStream().filter(dic -> !StrUtil.equalsAny(dic.getPath().getName(), ".meta", ".tmp"))
                    .collect(Collectors.toList());
            if (StringUtils.isNotBlank(partitionFiled)) {
                // 说明存在分区
                ...
                for (FileStatus uri : collect) {
                    ...
                    if (CollectionUtil.isNotEmpty(parquetFiles)) {
                        for (FileStatus parquetFile : parquetFiles) {
                            String name = parquetFile.getPath().getName();
                            if (!StringUtils.endsWithIgnoreCase(name, HadoopTableInfo.PARQUET_TYPE)) {
                                continue;
                            }
                            long len = parquetFile.getLen();
                            if (len > 0) {
                                Path parquetFilePath = parquetFile.getPath();
                                List<Map<String, Object>> list = readParquet(parquetFilePath, connect, queryEntity.getQuery().getLimit());
                                return new QueryMethodResult(list.size(), list);
                            }
                        }
                    }
    ...
    protected QueryMethodResult createTableMethod(S connect, Q queryEntity, D databaseConf, T tableConf) throws Exception {
        ...
        HadoopHandler.CreateTable createTable = queryEntity.getCreateTable();
        if (createTable != null) {
            String fileType = tableConf.getFileType();
            if (StrUtil.equalsAnyIgnoreCase(fileType, HadoopTableInfo.ORC_TYPE)) {
                // 获取数据
                List<Writable> orcData = createTable.getOrcData();
                StatsProvidingRecordWriter orcWriter = null;
                try {
                    if (connect.exists(databaseConf.getBashPath() + tableConf.getPath() + TABLE_META_DIC + HadoopTableInfo.ORC_TYPE)) {
                        throw new BusinessException(ExceptionCode.CREATE_TABLE_EXISTS_EXCEPTION);
                    }
                    // 获取写对象
                    orcWriter = getOrcWriter(connect, databaseConf.getBashPath() + tableConf.getPath() + TABLE_META_DIC + HadoopTableInfo.ORC_TYPE, tableConf.getCompressionCode(), tableConf.getBlockSize());
                ...
                try {
                    if (connect.exists(databaseConf.getBashPath() + tableConf.getPath() + TABLE_META_DIC + HadoopTableInfo.PARQUET_TYPE)) {
                        throw new BusinessException(ExceptionCode.CREATE_TABLE_EXISTS_EXCEPTION);
                    }
                    if(CollectionUtils.isNotEmpty(parquetSchemaData)) {
                        parquetWriter = getParquetWriter(connect.getConf(), databaseConf.getBashPath() + tableConf.getPath() + TABLE_META_DIC + HadoopTableInfo.PARQUET_TYPE, messageType, tableConf.getCompressionCode());
            ...
            // 清除当前表schema缓存
            connect.invalidateSchemaCache(databaseConf, tableConf);
        }
        return new QueryMethodResult();
    }
    ...
    protected QueryMethodResult dropTableMethod(S connect, Q queryEntity, D databaseConf, T tableConf) throws Exception {
        HadoopHandler.DropTable dropTable = queryEntity.getDropTable();
        ...
        if (startTime != null && stopTime != null) {
            long between = DateUtil.between(startTime, stopTime, DateUnit.DAY);
            String partitionPath;
            String format;
            if (StringUtils.isNotBlank(tableConf.getFormat())) {
                String _format = tableConf.getFormat();
                // 解析字段名
                partitionPath = StringUtils.substring(_format, 0, StringUtils.indexOf(_format, "="));
                // 解析格式
                format = StringUtils.substring(_format, StringUtils.indexOf(_format, "#") + 1, StringUtils.indexOf(_format, "}"));
            } else {
                // 读取 元数据文件解析 分区信息
                if (StrUtil.equalsIgnoreCase(tableConf.getFileType(), HadoopTableInfo.ORC_TYPE)) {
                    SchemaInfo<StandardStructObjectInspector> orcSchema = connect.getOrcSchema(databaseConf, tableConf);
                    partitionPath = orcSchema.getPath();
                    format = orcSchema.getFormat();
                } else {
                    SchemaInfo<MessageType> parquetSchema = connect.getParquetSchema(databaseConf, tableConf);
                    partitionPath = parquetSchema.getPath();
                    format = parquetSchema.getFormat();
                }
            }
            // 如果没有分区路径，则说明没有分区
            if (StringUtils.isBlank(partitionPath)) {
                throw new BusinessException(ExceptionCode.DB_PARAM_ERROR_DESC_EXCEPTION, "当前 HDFs 为非分区类型，不支持根据时间范围删除!");
            } else {
                // 因为 path 不会以 / 结尾
                if (!StringUtils.startsWithIgnoreCase(partitionPath, "/")) {
                    partitionPath = "/" + partitionPath;
                }
                if (StringUtils.isBlank(format)) {
                    format = DatePattern.NORM_DATE_PATTERN;
                }
                String finalPartitionPath = partitionPath;
                if (between == 0) {
                    String partitionValue = DateUtil.format(startTime, format);
                    // 删除前先关闭写入对象
                    dropTableCloseWriter(databaseConf, tableConf, path, partitionValue, () -> {
                        String delPath = path + finalPartitionPath + "=" + partitionValue;
                        log.info("delete file path: [{}]", delPath);
                        connect.delete(delPath);
                    });
                } else {
                    int dateType = 8;
                    if (format.length() == 8) {
                    ...
                    while (stopTime.after(startTime) || stopTime.equals(startTime)) {
                        String partitionValue = DateUtil.format(stopTime, format);
                        // 删除前先关闭写入对象
                        dropTableCloseWriter(databaseConf, tableConf, path, partitionValue, () -> {
                            String delPath = path + finalPartitionPath + "=" + partitionValue;
                            log.info("delete file path: [{}]", delPath);
                        ...
                        stopTime = com.meiya.whalex.util.date.DateUtil.add(stopTime, dateType, -1);
                    }
                }
            }
        } else {
            ...
            // 全表数据删除
            dropTableCloseWriter(databaseConf, tableConf, path, null, () -> {
     ...
     * @param callBack
     */
    private void dropTableCloseWriter(D databaseConf, T tableConf, String path, String partition, DeletePathCallBack callBack) throws Exception {
        if (StrUtil.equalsIgnoreCase(tableConf.getFileType(), HadoopTableInfo.ORC_TYPE)) {
            String cacheKey = helper.getCacheKey(databaseConf, tableConf);
            Cache<String, Map<String, OrcWriter>> orcWriterCache = this.orcWriterCache.get(cacheKey);
            if (orcWriterCache != null) {
                ConcurrentMap<String, Map<String, OrcWriter>> orcWriterMap = orcWriterCache.asMap();
                if (MapUtil.isNotEmpty(orcWriterMap)) {
                    if (StringUtils.isBlank(partition)) {
                        // 全表删除
                        Set<Map.Entry<String, Map<String, OrcWriter>>> collect = orcWriterMap.entrySet().stream().filter(entry -> {
                            String key = entry.getKey();
                            return StringUtils.startsWith(key, tableConf.getTableName());
                        }).collect(Collectors.toSet());

                        if (CollectionUtil.isNotEmpty(collect)) {
                            ReentrantLock reentrantLock = this.writeLockCache.get(tableConf.getTableName() + "$" + cacheKey);
                            // 关闭写对象
                            try {
                                reentrantLock.lock();
                                if (collect.size() == 1) {
                                    Map.Entry<String, Map<String, OrcWriter>> next = collect.iterator().next();
                                    String key = next.getKey();
                                    Map<String, OrcWriter> writerMap = next.getValue();
                                    String writeKey = writerMap.keySet().iterator().next();
                                    String partitionPath = StringUtils.split(writeKey, "$")[0];
                                    OrcWriter orcWriter = writerMap.values().iterator().next();
                                    closeOrcWriter(cacheKey, partitionPath, orcWriter.getWriter());
                                    orcWriterCache.invalidate(key);
                                } else {
                                    for (Map.Entry<String, Map<String, OrcWriter>> entry : collect) {
                                        String key = entry.getKey();
                                        Map<String, OrcWriter> writerMap = entry.getValue();
                                        String writeKey = writerMap.keySet().iterator().next();
                                        String partitionPath = StringUtils.split(writeKey, "$")[0];
                                        OrcWriter next = writerMap.values().iterator().next();
                                        closeOrcWriter(cacheKey, partitionPath, next.getWriter());
                                        orcWriterCache.invalidate(key);
                                    }
                                }
                            } catch (Exception e) {
                                log.error("写入对象流关闭失败! cacheKey: [{}] file: [{}]", cacheKey, path, e);
                            } finally {
                        ...
                        Map<String, OrcWriter> writerMap = orcWriterMap.get(tableConf.getTableName() + "$" + partition);
                        if (MapUtil.isNotEmpty(writerMap)) {
                            ReentrantLock reentrantLock = this.writeLockCache.get(tableConf.getTableName() + "$" + cacheKey);
                            // 关闭写对象
                            try {
                                reentrantLock.lock();
                                String key = writerMap.keySet().iterator().next();
                                String partitionPath = StringUtils.split(key, "$")[0];
                                OrcWriter next = writerMap.values().iterator().next();
                                closeOrcWriter(cacheKey, partitionPath, next.getWriter());
                                orcWriterCache.invalidate(tableConf.getTableName() + "$" + partition);
                            } catch (Exception e) {
                                log.error("写入对象流关闭失败! cacheKey: [{}] file: [{}]", cacheKey, path, e);
                            } finally {
                                reentrantLock.unlock();
            ...
            String cacheKey = helper.getCacheKey(databaseConf, tableConf);
            Cache<String, Map<String, ParquetWriter<Group>>> parquetWriterCache = this.parquetWriterCache.get(cacheKey);
            if (parquetWriterCache != null) {
                // 存在 写入对象则判断是否有当前删除对象的写入
                ConcurrentMap<String, Map<String, ParquetWriter<Group>>> parquetWriterMap = parquetWriterCache.asMap();
                if (MapUtil.isNotEmpty(parquetWriterMap)) {
                    // 存在当前删除对象的写入
                    if (StringUtils.isBlank(partition)) {
                        // 全表删除
                        Set<Map.Entry<String, Map<String, ParquetWriter<Group>>>> collect = parquetWriterMap.entrySet().stream().filter(entry -> {
                            String key = entry.getKey();
                            return StringUtils.startsWith(key, tableConf.getTableName());
                        }).collect(Collectors.toSet());
                        ...
                        if (CollectionUtil.isNotEmpty(collect)) {
                            ...
                            try {
                                ...
                                if (collect.size() == 1) {
                                    // 说明是无分区表
                                    Map.Entry<String, Map<String, ParquetWriter<Group>>> next = collect.iterator().next();
                                    String key = next.getKey();
                                    Map<String, ParquetWriter<Group>> writerMap = next.getValue();
                                    String writeKey = writerMap.keySet().iterator().next();
                                    String partitionPath = StringUtils.split(writeKey, "$")[0];
                                    ParquetWriter<Group> parquetWriter = writerMap.values().iterator().next();
                                    closeParquetWriter(cacheKey, partitionPath, parquetWriter);
                                    parquetWriterCache.invalidate(key);
                                } else {
                                    for (Map.Entry<String, Map<String, ParquetWriter<Group>>> entry : collect) {
                                        String key = entry.getKey();
                                        Map<String, ParquetWriter<Group>> writerMap = entry.getValue();
                                        String writeKey = writerMap.keySet().iterator().next();
                                        String partitionPath = StringUtils.split(writeKey, "$")[0];
                                        ParquetWriter<Group> next = writerMap.values().iterator().next();
                                        closeParquetWriter(cacheKey, partitionPath, next);
                                        parquetWriterCache.invalidate(key);
                                    }
                                }
                            } catch (Exception e) {
                        ...
                        Map<String, ParquetWriter<Group>> writerMap = parquetWriterMap.get(tableConf.getTableName() + "$" + partition);
                        if (MapUtil.isNotEmpty(writerMap)) {
                            ReentrantLock reentrantLock = this.writeLockCache.get(tableConf.getTableName() + "$" + cacheKey);
                            // 关闭写对象
                            try {
                                ...
                                ParquetWriter<Group> next = writerMap.values().iterator().next();
                                closeParquetWriter(cacheKey, partitionPath, next);
                                parquetWriterCache.invalidate(tableConf.getTableName() + "$" + partition);
                            } finally {
                                reentrantLock.unlock();
                            }
                        }
     ...
     * @throws Exception
     */
    private static FileTreeNode getFileTreeNode(HDFSClient connect, FilterFileCondition filter, String rootPath, List<FileStatus> fileStatusList, Integer hierarchy) throws Exception {
        // 文件大小过滤
    ...
    protected QueryMethodResult insertMethod(S connect, Q queryEntity, D databaseConf, T tableConf) throws Exception {
        ...
        if (StrUtil.equalsIgnoreCase(fileType, HadoopTableInfo.ORC_TYPE)) {
            // 若文件类型指定为 orc 则以 orc 格式作为元数据文件读取
            SchemaInfo<StandardStructObjectInspector> orcSchema = connect.getOrcSchema(databaseConf, tableConf);
            // 写文件
            writeOrc(connect, databaseConf, tableConf, orcSchema, rowData, cacheKey, lock, commitNow);
        } else {
            // 获取 schema
            SchemaInfo<MessageType> parquetSchema = connect.getParquetSchema(databaseConf, tableConf);
            // 写文件
            writeParquet(connect, databaseConf, tableConf, parquetSchema, rowData, cacheKey, lock, commitNow);
        }
    ...
    protected QueryMethodResult querySchemaMethod(S connect, D databaseConf, T tableConf) throws Exception {
        String schemaPath = databaseConf.getBashPath() + tableConf.getPath() + BaseHadoopServiceImpl.TABLE_META_DIC + tableConf.getFileType();
        List<Map<String, Object>> schema = new ArrayList<>();
        if (StrUtil.equalsIgnoreCase(tableConf.getFileType(), HadoopTableInfo.ORC_TYPE)) {
            RecordReader rows = null;
            try {
                Reader reader = OrcFile.createReader(connect.getFileSystem(), new Path(schemaPath));
                rows = reader.rows();
                while (rows.hasNext()) {
                    OrcStruct next = (OrcStruct) rows.next(null);
                    ...
                    IntWritable length = (IntWritable) getFieldValue.invoke(next, 2);
                    if (name != null) {
                        resultMap.put("col_name", name.toString());
                    }
                    if (type != null) {
                        resultMap.put("data_type", type.toString());
                    }
                    if (length != null) {
                        resultMap.put("columnSize", length.get());
                    }
                    schema.add(resultMap);
                }
            } catch (Exception e) {
            ...
            ParquetReader<Group> reader = null;
            try {
                reader = ParquetReader.builder(new GroupReadSupport(), new Path(schemaPath)).withConf(connect.getConf()).build();
                SimpleGroup group = null;
                ...
                while (true) {
                    ...
                    for (int i = 0; i < groupType.getFieldCount(); i++) {
                        ...
                        resultMap.put(fieldName.equalsIgnoreCase("name") ? "col_name" : fieldName.equalsIgnoreCase("type") ? "data_type" : "columnSize", value);
        ...
        return new QueryMethodResult(schema.size(), schema);
    }
    ...
    protected QueryMethodResult alterTableMethod(S connect, Q queryEntity, D databaseConf, T tableConf) throws Exception {
        String path = databaseConf.getBashPath() + tableConf.getPath();
        HadoopHandler.AlterTable alterTable = queryEntity.getAlterTable();
    ...
    protected QueryMethodResult mergeDataMethod(S connect, Q queryEntity, D databaseConf, T tableConf) throws Exception {
        ...
        if (!StrUtil.equalsIgnoreCase(tableConf.getFileType(), HadoopTableInfo.PARQUET_TYPE)) {
            throw new BusinessException(ExceptionCode.HDFS_MERGE_FILE_TYPE_EXCEPTION, HadoopTableInfo.PARQUET_TYPE);
        }

        HadoopHandler.MergeData mergeData = queryEntity.getMergeData();
    ...
    protected MergeDataResult mergeDataTask(S connect, String path, List<FileStatus> fileStatusList, long blockSize, String fileType, String compressionCode) throws Exception {
        ...
        try {
            ...
            for (FileStatus fileStatus : fileStatusList) {
                ...
                // parquet 文件
                if (StrUtil.equalsIgnoreCase(fileType, HadoopTableInfo.PARQUET_TYPE)) {
    ...
    protected void mergeDataTask(String taskId, Configuration configuration, String targetFile, String mergePath, MessageType schema, String compressionCode, long blockSize, Group group) throws Exception {
        ...
        if (parquetWriter == null) {
            ...
            String mergeFile = mergePath + "/" + UUID.randomUUID().toString().replaceAll("-", "") + "." + HadoopTableInfo.PARQUET_TYPE;
