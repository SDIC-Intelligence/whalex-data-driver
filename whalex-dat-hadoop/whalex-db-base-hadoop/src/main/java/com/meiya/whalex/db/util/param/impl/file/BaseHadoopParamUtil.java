...
import com.meiya.whalex.db.entity.file.HadoopHandler;
 ...
 */
@Slf4j
public class BaseHadoopParamUtil<Q extends HadoopHandler, D extends HadoopDatabaseInfo,
        T extends HadoopTableInfo> extends AbstractFileSystemParserUtil<Q, D, T> {
    @Override
    protected Q transitionListTableParam(QueryTablesCondition queryTablesCondition, D databaseConf) throws Exception {
        return (Q) new HadoopHandler();
    }
    ...
    protected Q transitionMergeDataParam(MergeDataParamCondition mergeDataParamCondition, D databaseConf, T tableConf) throws Exception {
        ...
        HadoopHandler.MergeData mergeData = new HadoopHandler.MergeData();
        ...
        hadoopHandler.setMergeData(mergeData);
        return (Q) hadoopHandler;
    }
    ...
    protected Q transitionCreateTableParam(CreateTableParamCondition createTableParamCondition, D databaseConf, T tableConf) throws Exception {
        ...
        if (StrUtil.equalsAnyIgnoreCase(fileType, HadoopTableInfo.ORC_TYPE)) {
            ...
            HadoopHandler.CreateTable createTable = HadoopHandler.CreateTable.builder().orcData(writableList).orcSchema(inspector).build();
            hadoopHandler.setCreateTable(createTable);
        } else {
            ...
            HadoopHandler.CreateTable createTable = HadoopHandler.CreateTable.builder().parquetData(rows).parquetSchema(metadataSchema).build();
            hadoopHandler.setCreateTable(createTable);
        }
        return (Q) hadoopHandler;
    }
    ...
    protected Q transitionAlterTableParam(AlterTableParamCondition alterTableParamCondition, D databaseConf, T tableConf) throws Exception {
        ...
        HadoopHandler.AlterTable alterTable = new HadoopHandler.AlterTable();
        alterTable.setNewTableName(newTableName);
        hadoopHandler.setAlterTable(alterTable);
        return (Q) hadoopHandler;
    }
    ...
    protected Q transitionQueryParam(QueryParamCondition queryParamCondition, D databaseConf, T tableConf) throws Exception {
        HadoopHandler.Query query = HadoopHandler.Query.builder().limit(10).build();
        HadoopHandler hadoopHandler = new HadoopHandler();
        hadoopHandler.setQuery(query);
        return (Q) hadoopHandler;
    }
    ...
    protected Q transitionInsertParam(AddParamCondition addParamCondition, D databaseConf, T tableConf) throws Exception {
        ...
        HadoopHandler.Insert insert = HadoopHandler.Insert.builder().rowData(fieldValueList).commitNow(addParamCondition.getCommitNow()).build();
        HadoopHandler hadoopHandler = new HadoopHandler();
        hadoopHandler.setInsert(insert);
        return (Q) hadoopHandler;
    }
    ...
    protected Q transitionDropTableParam(DropTableParamCondition dropTableParamCondition, D databaseConf, T tableConf) throws Exception {
        HadoopHandler.DropTable dropTable = new HadoopHandler.DropTable();
        HadoopHandler hadoopHandler = new HadoopHandler();
        hadoopHandler.setDropTable(dropTable);
    ...
    public Q queryFileTreeNodeParser(QueryFileTreeNode queryFileTreeNode, D databaseConf, T tableConf) throws Exception {
        HadoopHandler hadoopHandler = new HadoopHandler();
        hadoopHandler.setQueryFileTreeNode(queryFileTreeNode);
        return (Q) hadoopHandler;
    }
